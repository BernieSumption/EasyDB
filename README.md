...
<!---headline-demo--->
```swift
// Record types are defined as structs
struct Book: Record {
    var id = UUID()
...
Other developer-friendly features include:

- A safe SQL-based API that allows you perform more advanced queries and inserts than permitted by the type-safe API, while protecting you from SQL injection
- Thread-safety: safely use the database concurrently from multiple threads/tasks
- Structured data - record types can contain contain arbitrary Codable types that are serialised to JSON for storage
...
The `save(_:)` method on a collection will ensure that a record is persisted to the database:
...
EasyDB supports transactions. By default, every use of the API is wrapped in its own transaction. This can lead to inconsistency bugs, even when reading data. For example, this code selects a count and a sum from a table in two separate queries:
...
If another task updates the Employees table concurrently, the count and the sum will be taken from different versions of the data, so it will be possible to have nonsense results like "zero employees with a non-zero total salary".

To prevent this, wrap related queries in a `read` block:
...
When you don't need to write, use a `read` block as many of these can execute in parallel. A runtime error will be thrown if you try to write from within a `read` block.

If you need to write, use a `write` block. This example moves money between two accounts after checking that the source account has enough balance:
...
If a write block throws an error, the transaction will be rolled back and no data saved.
...
The EasyDB API is thread-safe provided that you **only create a single `EasyDB` instance for each database file**. You can use the database simultaneously from multiple tasks/threads.

EasyDB uses SQLite's [Write-Ahead Logging](https://www.sqlite.org/wal.html) feature to enable multiple concurrent readers and a single writer. Reads always execute immediately. Writes will block if another task is currently writing.

Because write blocks can not run in parallel, make sure that they run quickly. Avoid performing unnecessary CPU-intensive work inside a write block, and *really* avoid waiting for network requests inside the block.
