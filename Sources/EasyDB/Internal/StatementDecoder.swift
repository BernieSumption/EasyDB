...
private struct SingleRowSingleColumnDecoderImpl: Decoder {
    ...
    func container<Key: CodingKey>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> {
        throw EasyDBError.notImplemented(feature: "decoding 2D arrays of objects e.g. [[MyStruct]].self")
    }

    /// Treat the value as a JSON-encoded string and decode it into an array
    func unkeyedContainer() throws -> UnkeyedDecodingContainer {
        throw EasyDBError.notImplemented(feature: "decoding 3D array e.g. [[[T]]].self")
    }
